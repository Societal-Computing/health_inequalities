...
IRdata <- IRdata %>%
  mutate(fg_relig = 
           case_when(
             g118==1 & fg_heard==1 ~ 1 ,
             g118==0 & fg_heard==1 ~ 0,
             g118==3 & fg_heard==1 ~ NA,
             g118>=8 & fg_heard==1 ~ NA)) %>%
  set_value_labels(fg_relig = c("Yes" = 1, "No"=0)) %>%
...
IRdata <- IRdata %>%
  mutate(fg_cont = 
           case_when(
             g119==1 & fg_heard==1 ~ 1 ,
             g119==2 & fg_heard==1 ~ 0,
             g119>=3 & fg_heard==1 ~ NA)) %>%
  set_value_labels(fg_cont = c("Yes continue" = 1, "No, not continue"=0 )) %>%
...
MRdata <- MRdata %>%
  mutate(fg_relig = 
           case_when(
             ...
             mg118==3 & fg_heard==1 ~ NA,
             mg118>=8 & fg_heard==1 ~ NA)) %>%
  set_value_labels(fg_relig = c("Yes" = 1, "No"=0)) %>%
  set_variable_labels(fg_relig = "Opinion on whether female circumcision is required by their religion")
